...
**关于状态管理更深入的解释请查看[这里](./state_management.md)。在那里你将看到更多的例子，以及简单的阶段管理器和响应式状态管理器之间的区别**。
...
用别名导航到新页面。查看更多关于命名路由的详细信息[这里](./route_management.md#navigation-with-named-routes)
...
**关于别名路由，和对路由的低级控制，请看[这里](./route_management.md)**。
...
**关于依赖管理的更深入解释请看[此处](./dependency_management.md)**。
